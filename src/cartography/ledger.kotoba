(ns cartography.ledger
  "Append-only audit ledger for the ISCO-08 2165 cartography actor: entry
  construction, hash chaining, and verification.
  ...
  README called it immutable, but nothing in the value made a dropped or
  ...
  audit is trying to check.

  Separately, the ledger could not answer the one question the README's
  robotics premise turns on. Measured on the pre-change tree, running an
  escalated request through human approval and running a clean one
...
(defn summary
  "One line per entry, for operators and for `cartography.sim` output."
  [ledger]
  (str/join "\n"
            (for [e ledger]
